...
/**
 * Shared by every AR screen ([MeasureScreen], [ShapeMeasureScreen]): `true` once it is safe to
 * mount `ARSceneView`, `false` while the one-time warm-up delay is still running.
 *
 * Was only ever implemented in [MeasureScreen] originally — [ShapeMeasureScreen] mounted its own
 ...
 * rather than duplicating the flag and delay a second time.
 */
@Composable
