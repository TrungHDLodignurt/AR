...
class Homography(private val m: FloatArray) {
 ...
 */
fun computeHomography(src: List<Vec2>, dst: List<Vec2>): Homography? {
...
fun measureRealDistanceMm(h: Homography, a: Vec2, b: Vec2): Float {
    val pa = h.apply(a)
    val pb = h.apply(b)
    val dx = pb.x - pa.x
    val dy = pb.y - pa.y
