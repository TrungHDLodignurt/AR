package vn.apero.armeasure.ar.presentation.level
 ...
 *
 * @param onClose when non-null, shows a "✕" pill in a top bar that invokes it.
 */
@Composable
fun LevelScreen(modifier: Modifier = Modifier, onClose: (() -> Unit)? = null) {
    ...
    Box(modifier = modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
        ...
        // Only rendered when a host actually wants a close affordance — the null case (today's
        // only caller) keeps this screen exactly as chrome-free as it always was.
        if (onClose != null) {
            Box(
                modifier = Modifier
